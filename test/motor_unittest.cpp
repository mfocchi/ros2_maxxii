...
TEST(MotorTest, ConstructorWithParameters) {
    Motor motor(VELOCITY, TORQUE_CONSTANT, MAX_RPM, MAX_AMP);
...
TEST(MotorTest, SetAndGetActualCurrent) {
    Motor motor(TORQUE, TORQUE_CONSTANT, MAX_RPM, MAX_AMP);
...
TEST(MotorTest, SetAndGetDesiredCurrent) {
    Motor motor(TORQUE, TORQUE_CONSTANT, MAX_RPM, MAX_AMP);
    motor.setDesiredCurrent(1.0);
    EXPECT_EQ(motor.getDesiredCurrent(), 1.0);
    motor.setDesiredCurrent(MAX_AMP * 1.5);
    EXPECT_EQ(motor.getDesiredCurrent(), MAX_AMP);
}
...
TEST(MotorTest, SetAndGetDesiredTorque) {
    Motor motor(TORQUE, 2, MAX_RPM, MAX_AMP);
